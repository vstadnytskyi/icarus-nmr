...
class Event_Detector(object):
    ...
    def run(self):
        ...
        #self.packet_pointer = self.DAQ_packet_pointer
        #self.g_packet_pointer = self.DAQ_packet_pointer #get DAQ packet pointer upon start. Do not analyse events before.
    ...
    def run_once(self):
        ...
        while distance(back = self.packet_pointer,front = self.DAQ_packet_pointer,size = self.DAQ_packet_buffer_size) > 6:
            #debug('%r,<%r ' % (event_detector.packetPointer,daq.packetPointer))
            """"""
...
##            start = argwhere(event_buffer_vector[1,:] == 20)[-2][0]
    ...
    def analyse_pump_event(self, data = "", freq = 4000, pressure_threshold = 1000, gradient_threshold = -400*1000):
        from numpy import size, where, gradient,median, nanmedian, nan, isnan, nonzero, argmax,argmin,argwhere, max, sum, mean
        from time import time
        ...
        try:
            ...
            if grad_min < (gradient_threshold/freq) and median(target) > pressure_threshold:
                flag = True
            else:
                flag = False
...
###  Old codes
##########################################################################################
    def FWHM(X,Y): # Legacy code to be deleted?
        half_max = max(Y) / 2.
        #find when function crosses line half_max (when sign of diff flips)
        #take the 'derivative' of signum(half_max - Y[])
        d = sign(half_max - arreay(Y[0:-1])) - sign(half_max - array(Y[1:]))
...
##########################################################################################
    ...
    def get_cooling_traces_lst(self, folder = 'cooling_data/2019-04-12-19-05-23'):
        from numpy import genfromtxt
        import os
        ...
        temp_lst = []
        for item in dir_lst:
            if 'cooling' in item and '._' not in item:
                t_lst = item.split('_')
                t_lst.append(item)
                temp_lst.append(t_lst)

        lst2 = sorted(temp_lst, key=lambda x: x[1])
        return lst2

    def get_cooling_traces(self, lst, folder,N):
        from numpy import genfromtxt
        data = genfromtxt(folder + '/buffer_files/'+lst[N][3],delimiter = ',')
        x = data[:,0]
        sample = data[:,1]
        target = data[:,2]
...
if __name__ == "__main__":
    ...
    #test_buffer = self.test_ring_buffer(1)
